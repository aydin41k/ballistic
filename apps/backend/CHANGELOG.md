...
- **HTTPS asset URLs on Azure**: Added `trustProxies(at: '*')` to `bootstrap/app.php` so Laravel correctly detects the HTTPS scheme behind Azure's reverse proxy. Previously, all Vite asset URLs were generated as `http://` causing mixed-content blocking in browsers.
