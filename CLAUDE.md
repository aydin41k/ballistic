...
Always create models, controllers, factories, seeders, migrations through artisan commands. Never create a file yourself if there is a CMD command to do it.

Before wrapping up a task, update the changelog file accordingly. Assign a version. Increment either the tenth of the current version (0.1 to 0.2) or a hundredth, depending on how significant the work was. Don't touch the decimals, I decide that.

Use `sail artisan` to run artisan commands. DO NOT use `php artisan`
Use `sail bash` to run container commands
...
- When coding in PHP, use PHP Attributes where relevant.
...
## Testing Rules
- After each feature is developed or a bug is fixed, add a test to verify what you did.
- There is no jest, don't unit test the frontend.
- Frontend tests: Lint, build, etc.

## General Guidelines
- Never assume you know what the date and time is. Always check it via the CLI.
- Always use Australian spelling and vocabulary.
